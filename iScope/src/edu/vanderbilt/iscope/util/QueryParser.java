...
{
	public static Logger LOGGER = Logger.getLogger(QueryParser.class.toString());
	...
	public HashMap<String, String> parseGetParameters(String url) {
		...
		while(st.hasMoreTokens()) {
			...
			if(st1.hasMoreTokens()) {
				...
				LOGGER.debug("Adding GET pair ['"+value+"'] => ['"+name+"']");
			} else {
				params.put(name, "");
			}
		}
	...
	public HashMap<String, String> parseInputParameters(String paras) {
		LOGGER.debug("Clear REQUEST mapping.");
		...
		while (st.hasMoreTokens()) {
			...
			if (st1.hasMoreTokens()) {
				...
				LOGGER.debug("Adding REQUEST pair ['"+value+"'] => ['"+name+"']");
			} else {
				parameters.put(name, "");
			}
	...
	public HashMap<String, String> parseSession(String session) {
		...
		while (st.hasMoreTokens()){
			...
			if (!key.equals("") && !value.equals("")){
				...
				LOGGER.debug("Adding SESSION pair ['"+value+"'] => ['"+key+"']");
			}
		}
	...
	public String parse_to_sig(String str) {
		...
		try {
			...
			LOGGER.debug("REQUEST: {"+request_variables_inv.toString()+"}");
	...
	public SQLQuery parseQuery(String str) throws JSQLParserException {
		query.clear();
		//didSomeOutput = false;
		LOGGER.info("ENTER parseQuery: "+str);
		...
		LOGGER.debug("REQUEST: {"+request_variables_inv.toString()+"}");
		//if(didSomeOutput) LOGGER.debug(parsed_expression+"\n");
		int ctr = 0;
		if(sql_queries.containsKey(parsed_expression))
			ctr = sql_queries.get(parsed_expression).intValue();
		
		sql_queries.put(parsed_expression, ctr+1);
		LOGGER.info(parsed_expression);
		...
		if(LOGGER.isInfoEnabled() ) {
			...
			for(String f : query.getFields())
				sb.append("\t"+f+" : "+query.getValues().get(f)+"\n");
